...
class Kaltura_Client_ContentDistribution_Type_DistributionDeleteJobData extends Kaltura_Client_ContentDistribution_Type_DistributionJobData
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->keepDistributionItem))
		{
		...
		if(!is_null($jsonObject) && isset($jsonObject->keepDistributionItem))
		{
			if(!empty($jsonObject->keepDistributionItem) && ((int) $jsonObject->keepDistributionItem === 1 || strtolower((string)$jsonObject->keepDistributionItem) === 'true'))
				$this->keepDistributionItem = true;
			else
				$this->keepDistributionItem = false;
		}
