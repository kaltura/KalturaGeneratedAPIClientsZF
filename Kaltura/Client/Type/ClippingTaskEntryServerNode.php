...
class Kaltura_Client_Type_ClippingTaskEntryServerNode extends Kaltura_Client_Type_TaskEntryServerNode
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->clipAttributes) && !empty($xml->clipAttributes))
			$this->clipAttributes = Kaltura_Client_ParseUtils::unmarshalObject($xml->clipAttributes, "KalturaClipAttributes");
		if(!is_null($jsonObject) && isset($jsonObject->clipAttributes) && !empty($jsonObject->clipAttributes))
			$this->clipAttributes = Kaltura_Client_ParseUtils::jsObjectToClientObject($jsonObject->clipAttributes, "KalturaClipAttributes");
		if(!is_null($xml) && count($xml->clippedEntryId))
			$this->clippedEntryId = (string)$xml->clippedEntryId;
		if(!is_null($jsonObject) && isset($jsonObject->clippedEntryId))
			$this->clippedEntryId = (string)$jsonObject->clippedEntryId;
