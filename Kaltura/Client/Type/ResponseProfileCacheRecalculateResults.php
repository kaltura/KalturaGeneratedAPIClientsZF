...
class Kaltura_Client_Type_ResponseProfileCacheRecalculateResults extends Kaltura_Client_ObjectBase
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->lastObjectKey))
			$this->lastObjectKey = (string)$xml->lastObjectKey;
		if(!is_null($jsonObject) && isset($jsonObject->lastObjectKey))
			$this->lastObjectKey = (string)$jsonObject->lastObjectKey;
		if(!is_null($xml) && count($xml->recalculated))
			$this->recalculated = (int)$xml->recalculated;
		if(!is_null($jsonObject) && isset($jsonObject->recalculated))
			$this->recalculated = (int)$jsonObject->recalculated;
	}
	/**
