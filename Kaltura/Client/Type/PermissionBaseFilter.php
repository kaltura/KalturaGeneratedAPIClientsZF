 ...
 * @subpackage Client
 */
abstract class Kaltura_Client_Type_PermissionBaseFilter extends Kaltura_Client_Type_RelatedFilter
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->friendlyNameLike))
			$this->friendlyNameLike = (string)$xml->friendlyNameLike;
		if(!is_null($jsonObject) && isset($jsonObject->friendlyNameLike))
			$this->friendlyNameLike = (string)$jsonObject->friendlyNameLike;
		...
		if(!is_null($xml) && count($xml->dependsOnPermissionNamesMultiLikeOr))
			$this->dependsOnPermissionNamesMultiLikeOr = (string)$xml->dependsOnPermissionNamesMultiLikeOr;
		if(!is_null($jsonObject) && isset($jsonObject->dependsOnPermissionNamesMultiLikeOr))
			$this->dependsOnPermissionNamesMultiLikeOr = (string)$jsonObject->dependsOnPermissionNamesMultiLikeOr;
		if(!is_null($xml) && count($xml->dependsOnPermissionNamesMultiLikeAnd))
			$this->dependsOnPermissionNamesMultiLikeAnd = (string)$xml->dependsOnPermissionNamesMultiLikeAnd;
		if(!is_null($jsonObject) && isset($jsonObject->dependsOnPermissionNamesMultiLikeAnd))
			$this->dependsOnPermissionNamesMultiLikeAnd = (string)$jsonObject->dependsOnPermissionNamesMultiLikeAnd;
